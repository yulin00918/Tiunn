...
const unicodeNFC = v => typeof v === "string" ? v.normalize("NFC") : v;
const unicodeText = v => unicodeNFC(String(v ?? ""));
    ...
    if (request.method === "POST" && url.pathname === "/api/rooms") {
      for (let attempt = 0; attempt < 12; attempt++) {
        ...
        if (!info.active) {
          ...
          const title = unicodeText(body.title || "今天的互動活動").slice(0, 80);
  ...
  async webSocketMessage(ws, message) {
    ...
    if (info.role === "host") {
      if (data.type === "publish") {
        ...
        const normalizeDeep = value => {
          if (typeof value === "string") return unicodeNFC(value);
          if (Array.isArray(value)) return value.map(normalizeDeep);
          if (value && typeof value === "object") {
            return Object.fromEntries(Object.entries(value).map(([k,v]) => [k, normalizeDeep(v)]));
          }
          return value;
        };
        const activity = {
          ...
          question: unicodeText(payload.question || "").slice(0, 500),
          settings: normalizeDeep(payload.settings || {}),
          status: "open",
    ...
    if (data.type === "board_template_set" && info.role === "host") {
      ...
      if (payload.type === "classify") {
        ...
        const labels = Array.isArray(payload.labels)
          ? payload.labels.slice(0, count).map(x => unicodeText(x).slice(0, 40))
          : [];

        board.template = {
          type: "classify",
          title: unicodeText(payload.title || "分類活動").slice(0, 100),
          count,
          labels,
          updatedAt: Date.now(),
        };
      ...
      if (payload.type === "sort") {
        board.template = {
          type: "sort",
          title: unicodeText(payload.title || "排序活動").slice(0, 100),
      ...
      if (payload.type === "quad") {
        board.template = {
          type: "quad",
          title: unicodeText(payload.title || "四象限活動").slice(0, 100),
      ...
      if (payload.type === "match") {
        board.template = {
          type: "match",
          title: unicodeText(payload.title || "配對活動").slice(0, 100),
      ...
      if (payload.type === "flow") {
        board.template = {
          type: "flow",
          title: unicodeText(payload.title || "流程活動").slice(0, 100),
      ...
      if (payload.type === "imageMark") {
        board.template = {
          type: "imageMark",
          title: unicodeText(payload.title || "圖片標註活動").slice(0, 100),
      ...
      if (payload.type === "decision") {
        const criteria = Array.isArray(payload.criteria)
          ? payload.criteria.slice(0, 6).map(x => unicodeText(x).slice(0, 40))
          : [];
        board.template = {
          type: "decision",
          title: unicodeText(payload.title || "選擇決策").slice(0, 100),
      ...
      if (payload.type === "task") {
        board.template = {
          type: "task",
          title: unicodeText(payload.title || "今日任務").slice(0, 100),
    ...
    if (["board_object_add", "board_object_move", "board_object_update", "board_object_delete"].includes(data.type)) {
      ...
      if (data.type === "board_object_update") {
        ...
        existing.text = unicodeText(payload.text || "").slice(0, 1500);
        await this.ctx.storage.put("board", board);
