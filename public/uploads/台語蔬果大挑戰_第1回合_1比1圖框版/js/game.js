...
function choose(ans, btn){
  ...
  if(ans===x.roman){
    ...
    $("feedbackDetail").innerHTML=`華語：<strong>${x.zh}</strong><br>台文：<strong>${x.word}</strong><br>台羅：<strong>${x.roman}</strong>`;
...
function quizTimeout(){
  ...
  $("feedbackDetail").innerHTML=`正確答案<br>華語：<strong>${x.zh}</strong><br>台文：<strong>${x.word}</strong><br>台羅：<strong>${x.roman}</strong>`;
  $("feedback").classList.add("show");
  $("nextBtn").classList.remove("hidden");
  unlockAfterQuestion();
}
