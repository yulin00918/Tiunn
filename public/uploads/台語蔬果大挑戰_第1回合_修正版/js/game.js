...
function choose(ans, btn){
  ...
  if(ans===x.roman){
    ...
    $("feedbackDetail").innerHTML=`<div class="answerLines"><div>華語：<strong>${x.zh}</strong></div><div>台文：<strong>${x.word}</strong></div><div>台羅：<strong>${x.roman}</strong></div></div>`;
...
function quizTimeout(){
  ...
  $("feedbackDetail").innerHTML=`<div><strong>⏰ 時間到！正確答案</strong></div><div class="answerLines"><div>華語：<strong>${x.zh}</strong></div><div>台文：<strong>${x.word}</strong></div><div>台羅：<strong>${x.roman}</strong></div></div>`;
  $("feedback").classList.add("show");
  $("nextBtn").classList.remove("hidden");
  unlockAfterQuestion();
}
...
function revealRapid(){
  ...
  $("rapidZh").innerHTML=`華語：<strong>${x.zh}</strong><br>台文：<strong>${x.word}</strong><br>台羅：<strong>${x.roman}</strong>`;
